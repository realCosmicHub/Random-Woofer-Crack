...
rem Hostname change
SET "TextFile=C:\Riot\Beta\host.txt"
...
rem MACadress
SET "TextFile=C:\Riot\Beta\mac.txt"
FOR /f %%a IN ('type "%textfile%"^|find /c /v ""') DO SET /a numlines=%%a
SET /A RandomLine=(%RANDOM% %% %NumLines%)
IF "%RandomLine%"=="0" (SET "RandomLine=") ELSE (SET "RandomLine=skip=%randomline%")
FOR /F "usebackq tokens=* %RandomLine% delims=" %%A IN (`TYPE %TextFile%`) DO (
...
rem VolumeID change
C:\Riot\Beta\Volumeid64.exe /accepteula
C:\Riot\Beta\Volumeid64.exe 
C:\Riot\Beta\Volumeid64.exe /accepteula
C:\Riot\Beta\Volumeid64.exe C: !_RndAlphaNum!0-!_RndAlphaNum2!
